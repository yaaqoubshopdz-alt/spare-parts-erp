...
app.whenReady().then(() => {
  const os = require('os');
  const dbDir = join(os.homedir(), 'AppData', 'Roaming', 'spare-parts-erp', 'SparePartsERP');
  const dbPath = join(dbDir, 'spare_parts.db');
  ...
  try {
    ...
    // Delete existing products to avoid clutter (Optional but good since user wants realistic only)
    console.log('Keeping existing products, just adding new ones.');
    ...
    if (!pieceUnit) {
      const info = db.prepare('INSERT INTO units (code, name, symbol) VALUES (?, ?, ?)').run('pcs', 'قطعة', 'pcs');
      pieceUnit = { id: info.lastInsertRowid };
    }
    
    let literUnit = db.prepare('SELECT id FROM units WHERE name = ?').get('لتر');
    if (!literUnit) {
      const info = db.prepare('INSERT INTO units (code, name, symbol) VALUES (?, ?, ?)').run('l', 'لتر', 'L');
    ...
    for (const prod of productsList) {
      ...
      const pInfo = insertProdStmt.run(
        prod.name,
        barcode,
        code,
      ...
      const prodId = pInfo.lastInsertRowid;
