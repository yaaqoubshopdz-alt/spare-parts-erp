...
export default function SupplierStatement({ isOpen, onClose, supplierId }: SupplierStatementProps) {
  ...
  }, [isOpen, supplierId, dateFrom, dateTo]);

  const loadStatement = async () => {
    setLoading(true);
    try {
      const res = await window.electronAPI.invoke('db:suppliers:getStatement', supplierId, dateFrom || undefined, dateTo || undefined);
      if (res.success) {
  ...
  const navigateToInvoice = (t: any) => {
    if (t.reference_type === 'purchase_invoice' && t.reference_id) {
      navigate(`/purchases?invoiceId=${t.reference_id}`);
    } else {
      showError('هذه الحركة ليست فاتورة شراء');
    }
  };
      ...
      <div className="bg-background_primary border border-border_default rounded-2xl w-full max-w-6xl h-[90vh] flex flex-col shadow-2xl overflow-hidden">
          ...
          <div>
            ...
            {supplier && (
              <p className="text-sm text-text_secondary mt-1">
                {supplier.name} | {supplier.code} | هاتف: {supplier.phone || '-'}
              </p>
            )}
          </div>
        ...
        <div className="p-4 border-b border-border_default flex items-center gap-4 bg-background_primary">
          ...
          <div className="mr-auto">
            <span className="text-sm text-text_secondary ml-2">الرصيد المتبقي له (الديون علينا):</span>
            <span className={`font-bold font-numbers text-xl ${supplier?.balance > 0 ? 'text-danger_red' : 'text-success_green'}`}>
              {supplier?.balance?.toFixed(2) || '0.00'} د.ج
            </span>
          </div>
        </div>
          ...
          <table className="w-full text-sm text-right border-collapse">
            <thead className="sticky top-0 z-30 bg-gradient-to-b from-table_header_from to-table_header_to border-b border-black/30 dark:border-border_default shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
              <tr className="h-[52px]">
                ...
                <th className="px-4 py-3 font-bold text-[13px] text-text_primary tracking-wide border-l border-black/30 dark:border-border_default">المرجع</th>
                <th className="px-4 py-3 font-bold text-[13px] text-text_primary tracking-wide border-l border-black/30 dark:border-border_default">البيان (النوع)</th>
                <th className="px-4 py-3 font-bold text-[13px] text-text_primary tracking-wide border-l border-black/30 dark:border-border_default text-center">مدين (دفعنا له)</th>
                <th className="px-4 py-3 font-bold text-[13px] text-text_primary tracking-wide border-l border-black/30 dark:border-border_default text-center">دائن (اشترينا منه)</th>
                ...
                <th className="px-4 py-3 font-bold text-[13px] text-text_primary tracking-wide text-center">الإجراء</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border_default">
              {loading ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-text_muted">جاري التحميل...</td></tr>
              ) : formattedTransactions.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-text_muted">لا توجد حركات مالية في هذه الفترة</td></tr>
              ) : (
                formattedTransactions.map((t, idx) => (
                  <tr key={idx} className="hover:bg-background_card transition-colors group h-11">
                    ...
                    <td className="px-4 py-2 font-numbers text-text_primary font-bold border-l border-border_default">{t.reference || t.entry_number}</td>
                    <td className="px-4 py-2 border-l border-border_default">
                      {isPaymentRow(t) ? (
                        editingPaymentId === t.reference_id ? (
                          ...
                          <span className="text-success_green font-medium">دفع نقدية للمورد</span>
                        )
                      ) : (
                        <span className="text-primary_blue font-medium">فاتورة شراء</span>
                      )}
                    </td>
