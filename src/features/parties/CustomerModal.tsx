/**
 * CustomerModal — نافذة إضافة وتعديل زبون
 */
...
interface CustomerModalProps {
  ...
  onClose: () => void;
  onSaved: () => void;
  customer?: any | null; // Customer object if editing
}
...
export default function CustomerModal({ isOpen, onClose, onSaved, customer }: CustomerModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      if (customer) {
        ...
        // Auto-generate code based on timestamp for new customers
        const genCode = `CUS-${Date.now().toString().slice(-6)}`;
        setFormData({
          code: genCode,
          name: '', phone: '', phone2: '', address: '', email: '',
          credit_limit: 0, notes: '', is_active: true,
          initial_balance: 0,
        });
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (customer) {
        res = await window.electronAPI.invoke('db:customers:update', customer.id, formData);
      } else {
        res = await window.electronAPI.invoke('db:customers:create', formData);
      }

      if (res.success) {
        showSuccess(customer ? 'تم تعديل الزبون بنجاح' : 'تم إضافة الزبون بنجاح');
        onSaved();
        onClose();
      } else {
      ...
      <div className="bg-background_primary border border-border_default rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-center justify-between p-5 border-b border-border_default">
          <h2 className="text-xl font-bold text-text_primary flex items-center gap-2">
            <UserPlus size={22} className="text-primary_blue" />
        ...
        <div className="flex-1 overflow-auto p-6 custom-scrollbar">
          <form id="customer-form" onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="block text-sm text-text_secondary mb-2">الاسم الكامل <span className="text-danger_red">*</span></label>
                <input
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm text-text_secondary mb-2">سقف الدين (الائتمان)</label>
                <input
                  type="number" min="0" step="0.01"
                  value={formData.credit_limit}
                  onChange={e => setFormData({...formData, credit_limit: Number(e.target.value)})}
                  className="w-full bg-background_secondary border border-border_default rounded-xl px-4 py-2.5 text-text_primary outline-none focus:border-warning_amber font-numbers"
                />
              </div>
            </div>
            ...
            {!customer && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-text_secondary mb-2">الرصيد الافتتاحي (دين سابق إن وجد)</label>
                  <input
                    type="number" min="0" step="0.01"
